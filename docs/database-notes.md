...
- The latest applied migration is `V19`. The next new migration must be `V20`.
...
- The fourteenth Flyway migration (`V14__create_myplan_diet.sql`) is applied — `/myplan` Phase 2 (Diet), implemented end to end:
  ...
  - See `docs/dev-agent-plan.md` for the full design rationale, including why a fully autonomous AI moderator was considered and rejected in favor of a lightweight "look this up" search-link assist for human moderators.
...
- **No weight column on this table at all.** Logging a dose can optionally include a weight; this feature does not own its own copy of weight data. Displaying weight alongside a dose is a live join (by date) against the fetched `weight_entries` list at render time, not a stored/denormalized value, so the two can never drift out of sync. See `docs/dev-agent-plan.md` ("Phase 4 — GLP-1") for the full design discussion, including the "change since start" computation (also purely client-side, never persisted — reference point is `user_weight_goals.start_weight` if set, else the earliest `weight_entries` row, else no reference exists yet). **How a dose's optional weight is written changed in V19 — see below; this paragraph describes the original V16 design, kept for history.**

### V17 — avatar bytes moved to `user_avatars` — implemented

The seventeenth Flyway migration (`V17__move_avatar_bytes_to_user_avatars.sql`) is part of the 2026-07-18 performance-hardening pass (see `docs/dev-agent-plan.md`, "Performance Hardening"):
...
### V19 — link `weight_entries` to `medication_log_entries` — implemented

The nineteenth Flyway migration (`V19__link_weight_entries_to_medication_log.sql`), 2026-07-20, addressed a real gap reported by the owner: deleting a GLP-1 entry with an attached weight left the weight point stranded on the chart forever, since `weight_entries` and `medication_log_entries` were completely decoupled (V16 design) and neither table had a delete/edit endpoint for individual weight entries at all.

- `weight_entries.source_medication_entry_id` — nullable UUID FK → `medication_log_entries(id)` `ON DELETE CASCADE`, plus an index. Set **only** when adding a GLP-1 entry *creates* a new weight row for that date (`MyPlanGlp1Service.addEntry` → `MyPlanWeightService.upsertEntryForDate`) — left `NULL` when it instead updates a weight row that already existed (added independently, or by an earlier GLP-1 entry). This is the ownership rule the owner asked for: "the link i want is when i add [a weight] entry and then add the same date in glp1 it doesn't duplicate the entry... if we add the entry in glp1 it can be deleted and it will go away from chart as weight entry too" — but never for a weight that wasn't created by that GLP-1 entry.
- `POST /api/myplan/weight/entries` changed from reject-on-duplicate-date (`409`) to **upsert** (update in place, still `201`) — the old 409 was the literal bug report ("i tried to add same date enterie i got warning... current logic is not correct clearly"). The same upsert path is shared by the GLP-1 add flow via `MyPlanWeightService.upsertEntryForDate(user, date, weightKg, sourceMedicationEntry)`.
- New `PATCH`/`DELETE /api/myplan/weight/entries/{id}` — Weight tracking (V13) was the only `/myplan` sub-feature with no edit/delete for individual entries at all (Diet/Workout/GLP-1 all have it). `MyPlanView.tsx` gained an entries list (previously only the chart existed) with edit/delete, styled like the GLP-1/Workout flat row pattern.
- **Hibernate gotcha** (same class as `DietMealItem.clearFood()` for `foods`' `ON DELETE SET NULL`): the DB-level `ON DELETE CASCADE` alone isn't enough. If the linked `WeightEntry` is already loaded in the current persistence context (e.g. from the same request that just created it via `upsertEntryForDate`) when the owning `MedicationLogEntry` is deleted, a later query in the same transaction throws `TransientPropertyValueException` on the now-dangling `sourceMedicationEntry` reference — Hibernate's session doesn't know the DB already removed that row. Fix: `MyPlanGlp1Service.deleteEntry` calls `MyPlanWeightService.deleteLinkedWeightEntry(entry)` explicitly *before* `medicationLogEntryRepository.delete(entry)`, so Hibernate manages the removal itself instead of relying purely on the DB constraint.
- Backend suite grew from 266 to **276 tests**.
